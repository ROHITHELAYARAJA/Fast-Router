...
async function hasValidCliToken(request) {
  const token = request.headers.get("x-fast-router-cli-token") || request.headers.get(CLI_TOKEN_HEADER);
...
export async function proxy(request) {
  ...
  // Runtime APIs: accessible locally, with API key, CLI token, or dashboard session
  if (pathname.startsWith("/api/runtime")) {
    if (isLocalRequest(request) || await hasValidApiKey(request) || await hasValidCliToken(request) || await isAuthenticated(request)) {
      return NextResponse.next();
    }
  }
