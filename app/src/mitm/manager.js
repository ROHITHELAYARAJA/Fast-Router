...
const DEFAULT_MITM_ROUTER_BASE = "http://localhost:20201";
...
// (prevents EBUSY on `npm i -g Fast-Router@latest` while MITM is running).
...
const ENCRYPT_SALT = "Fast-Router-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall Fast-Router.`);
    }
  }
