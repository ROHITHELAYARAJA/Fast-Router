...
const ROOT_CA_CN = "Fast-Router MITM Root CA";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "Fast-Router MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const config = getLinuxCertConfig();
  const destFile = `${config.dir}/Fast-Router-root-ca.crt`;
