...
// Check if settings has Fast-Router config
const hasFastRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["Fast-Router"];
};

// Read per-agent models.json and return current model id (without "Fast-Router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["Fast-Router"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasFastRouter: hasFastRouterConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["Fast-Router"] = {
    baseUrl,
...
// POST - Update Fast-Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `Fast-Router/${model}`;

    // Remove all old Fast-Router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("Fast-Router/"))
    ...
    // Add fresh Fast-Router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`Fast-Router/${m}`] = {};
    });

    // Remove old Fast-Router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("Fast-Router/")) {
    ...
    // Update models.providers.Fast-Router with all models
    settings.models.providers["Fast-Router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `Fast-Router/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Fast-Router settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Fast-Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["Fast-Router"];
    ...
    // Remove Fast-Router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("Fast-Router/"));
    ...
    // Reset agents.defaults.model.primary if it uses Fast-Router
    if (settings.agents?.defaults?.model?.primary?.startsWith("Fast-Router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Fast-Router settings removed successfully",
    });
  } catch (error) {
