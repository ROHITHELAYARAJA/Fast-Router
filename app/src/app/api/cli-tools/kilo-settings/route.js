...
const hasFastRouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["Fast-Router"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("Fast-Router");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasFastRouter: hasFastRouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Fast-Router", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Fast-Router settings removed from Kilo Code" });
  } catch (error) {
