...
const hasFastRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Fast-Router");
};

const getFastRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Fast-Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getFastRouterEntry(config);
...
// POST - Apply Fast-Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Fast-Router entry or append
    const idx = config.findIndex((e) => e.name === "Fast-Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Fast-Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Fast-Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Fast-Router removed from Copilot config",
    });
  } catch (error) {
