...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-Fast-Router.env");
};
...
const hasFastRouterConfig = (config) => {
  ...
  if (providers["Fast-Router"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:20200")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasFastRouter = hasFastRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_FastRouter_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile Fast-Router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["Fast-Router"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_FastRouter_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "Fast-Router configuration removed from jcode",
    });
  } catch (error) {
