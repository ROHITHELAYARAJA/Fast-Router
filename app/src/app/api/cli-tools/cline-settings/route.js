...
const hasFastRouterConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("Fast-Router"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasFastRouter: hasFastRouterConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Fast-Router settings removed from Cline" });
  } catch (error) {
