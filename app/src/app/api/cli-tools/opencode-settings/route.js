...
const hasFastRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["Fast-Router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["Fast-Router"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasFastRouter: hasFastRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("Fast-Router/") ? config.model.replace(/^Fast-Router\//, "") : null,
...
// POST - Apply Fast-Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_FastRouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing Fast-Router provider entry and its models
    const existingProvider = config.provider["Fast-Router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["Fast-Router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `Fast-Router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `Fast-Router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("Fast-Router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Fast-Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["Fast-Router"]?.models) {
      delete config.provider["Fast-Router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["Fast-Router"].models).length === 0) {
        delete config.provider["Fast-Router"];
        if (config.model?.startsWith("Fast-Router/")) delete config.model;
      } else if (config.model === `Fast-Router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["Fast-Router"].models);
        config.model = `Fast-Router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire Fast-Router provider
      if (config.provider) delete config.provider["Fast-Router"];
      if (config.model?.startsWith("Fast-Router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("Fast-Router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Fast-Router settings removed from OpenCode",
    });
  } catch (error) {
