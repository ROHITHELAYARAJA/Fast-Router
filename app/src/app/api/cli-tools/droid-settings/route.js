...
// Check if settings has Fast-Router customModels
const hasFastRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Fast-Router"));
};
...
// POST - Update Fast-Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Fast-Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Fast-Router"));
...
// DELETE - Remove Fast-Router customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Fast-Router customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Fast-Router"));
