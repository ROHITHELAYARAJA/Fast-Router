...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_FastRouter for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# Fast-Router Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "Fast-Router"

[model_providers.Fast-Router]
name = "Fast-Router"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[model_providers.Fast-Router.http_headers]
