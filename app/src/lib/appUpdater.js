...
// Collect PIDs of all Fast-Router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from Fast-Router install dir or wrapper cli.js
        const isAppProcess = lower.includes("Fast-Router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("Fast-Router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "Fast-Router");
  }
  return path.join(os.homedir(), ".Fast-Router");
}
...
// Resolve npx/Fast-Router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx Fast-Router` — works regardless of global bin path changes after npm i -g
