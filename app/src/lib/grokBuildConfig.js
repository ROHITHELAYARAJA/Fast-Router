...
const previousDefaultRegExp = /^# Fast-Router-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# Fast-Router-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Fast-Router gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# Fast-Router-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# Fast-Router-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
