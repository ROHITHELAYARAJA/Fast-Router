...
function ensureClientToolsScript() {
  const scriptPath = path.join(os.tmpdir(), "Fast-Router-devin-client-tools.mjs");
  ...
  async execute({ model, body, credentials, signal, log }) {
    ...
    const sseStream = new ReadableStream({
      start(controller) {
        ...
        // modify the filesystem on the host running Fast-Router — only expose locally.
        ...
        sendRpc("initialize", {
          protocolVersion: "0.3",
          clientInfo: { name: "Fast-Router", version: "1.0" },
          capabilities: {},
        });
      },
    });
