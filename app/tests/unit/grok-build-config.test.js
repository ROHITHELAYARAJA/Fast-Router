...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("Fast-Router");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "Fast-Router-general-purpose",
      explore: "Fast-Router-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.Fast-Router\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.Fast-Router-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.Fast-Router-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# Fast-Router-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.Fast-Router-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("Fast-Router-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.Fast-Router-");
    expect(reset).not.toContain("Fast-Router-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("Fast-Router-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("Fast-Router-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("Fast-Router-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("Fast-Router-explore");
    expect(getGrokSubagentSlot("plan")).toBe("Fast-Router-plan");
