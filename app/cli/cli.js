...
// Subcommands (`Fast-Router xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.fast-router/runtime
...
// Identifiers for killAllAppProcesses - only kill Fast-Router specifically
const PROCESS_IDENTIFIERS = [
  'Fast-Router'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "fast-router")
    : path.join(os.homedir(), ".fast-router");
}
...
// Kill all Fast-Router processes
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running Fast-Router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "Fast-Router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("Fast-Router") && (cmd.includes("cli.js") || cmd.includes("\\Fast-Router") || cmd.includes("/Fast-Router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running Fast-Router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "Fast-Router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("Fast-Router") && (cmd.includes("cli.js") || cmd.includes("/Fast-Router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
// Find server: check bundled standalone (in published package), then repo custom-server.js
...
if (!fs.existsSync(serverPath)) {
  const repoCustomServer = path.resolve(__dirname, "..", "custom-server.js");
  if (fs.existsSync(repoCustomServer)) {
    standaloneDir = path.resolve(__dirname, "..");
    serverPath = repoCustomServer;
  } else {
    console.error("Error: Standalone build not found.");
    console.error("Please run 'npm run build:cli' or 'npm run build' first.");
    process.exit(1);
  }
}
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Fast-Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Fast-Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "Fast-Router", "db.json") : path.join(".fast-router", "db.json"));
