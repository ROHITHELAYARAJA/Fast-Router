...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `Fast-Router (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[Fast-Router] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[Fast-Router] tray init error: ${err.message}\n`);
    return null;
  }
}
