...
async function buildCodexHeader() {
  ...
  const { installed, hasFastRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasFastRouter, settings } = result.data;
  ...
  // Extract Fast-Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Fast-Router-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Fast-Router provider config
  const provider = settings?.models?.providers?.["Fast-Router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("Fast-Router/") ? primary.replace("Fast-Router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasFastRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Fast-Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
